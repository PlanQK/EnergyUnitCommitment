...
class AbstractDwaveSampler(BackendBase):
    ...
    def transform_problem_for_optimizer(self) -> None:
        ...
        self.ising_backbone = QuboTransformator.transform_network_to_qubo(self.network,
...
class DwaveCloudDirectQPU(DwaveCloudSampler):
    ...
    def get_sampler(self) -> dimod.Sampler:
        ...
        direct_sampler = DWaveSampler(solver={
            'qpu': True,
            'topology__type': 'pegasus'
        },
            token=self.token)
        if hasattr(self, 'embedding'):
            self.sampler = FixedEmbeddingComposite(direct_sampler,
                                                   self.embedding)
        else:
    ...
    def build_flow_problem(self,
        ...
        if line_values is not None:
            ...
            for bus in self.network.buses.index:
                generated_power = \
                    self.ising_backbone.calc_total_power_generated_at_bus(
                        bus, generator_state)
                load_name = self.network.loads.index[
    ...
    def save_sample(self, sampleset: dimod.SampleSet) -> None:
        ...
        self.output["results"]["optimization_time"] = \
            self.output["results"]["serial"]["info"]["timing"][
                "qpu_access_time"] / (10.0 ** 6)
        self.output["results"]["anneal_read_ratio"] = float(
            self.config["backend_config"]["annealing_time"]) / float(
            self.config["backend_config"]["num_reads"])
...
class DwaveReadQPU(DwaveCloudDirectQPU):
    ...
    def get_sampler(self) -> None:
        ...
        """
        self.input_file_path = "/energy/results_qpu/" + \
                               self.config["backend_config"]["sample_origin"]
