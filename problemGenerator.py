...
def main():
    ...
    for numBuses in range(5,25,1):
        CONFIG["numBuses"] = numBuses
        for i in range(int(sys.argv[1])):
            if ospath.isfile(f"TestProblems/nocostinput_{CONFIG['numBuses']}_{i}_{CONFIG['lineScale']}.nc"):
                print(f"skipping: {CONFIG['numBuses']}_{i}_{CONFIG['lineScale']}")
                continue
            newProblem = ProblemInstance()
            newProblem.exportNetwork(
                f"TestProblems/nocostinput_{CONFIG['numBuses']}_{i}_{CONFIG['lineScale']}.nc"
            )
    return
